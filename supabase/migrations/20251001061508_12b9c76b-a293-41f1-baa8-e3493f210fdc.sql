...
BEGIN
  ...
  DROP POLICY IF EXISTS "users_no_direct_select" ON public.users;
  DROP POLICY IF EXISTS "users_no_direct_update" ON public.users;
...
CREATE POLICY "users_no_direct_access" ON public.users
AS RESTRICTIVE
FOR ALL 
USING (false)
WITH CHECK (false);
